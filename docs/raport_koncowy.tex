% =============================================================================
% DDDS — system akustycznej detekcji UAV w czasie rzeczywistym
...
% Helper: include figure if file exists, else render a labelled placeholder.
% Two-arg form keeps the macro insensitive to underscores in filenames
% (a literal `_` in \texttt would otherwise trigger math mode).
\newcommand{\figureifexists}[2]{%
  \IfFileExists{#1}%
    {\includegraphics[width=#2]{#1}}%
    {\fbox{\parbox[c][6cm][c]{#2}{\centering\itshape\color{codecomment}%
      [placeholder --- zapisz brakujący zrzut ekranu i przekompiluj]}}}%
}
...
\fancyfoot[L]{\footnotesize dr inż. R. Dzierżak}
\fancyfoot[C]{\thepage}
...
{\normalsize Wydział Elektrotechniki i Informatyki}\\[0.2cm]
...
{\normalsize Rok akademicki 2025/2026}\\[1.0cm]

\rule{\linewidth}{0.5mm}\\[0.5cm]
{\Huge\bfseries\color{accent} DDDS}\\[0.20cm]
{\Large\bfseries\color{accent2} Distributed Drone Detection System}\\[0.35cm]
...
{\Large\itshape Sprawozdanie z projektu --- raport końcowy}\\[2.5cm]

\begin{flushleft}
\large
\textbf{Autor:}\hspace{1cm} Oleh Kropyva\\[0.3cm]
\textbf{Grupa:}\hspace{1.2cm} IZIwE II --- grupa 1.1.2\\[0.3cm]
\textbf{Prowadzący:}\hspace{0.2cm} dr inż.\ Róża Dzierżak\\[0.3cm]
\textbf{Data oddania:}\hspace{0.1cm} maj 2026\\[0.3cm]
\textbf{Repozytorium GitHub:} \url{https://github.com/hapole0n/DDDS-Distributed-Drone-Detection-System}
\end{flushleft}
\end{titlepage}
...
detekcji bezzałogowych statków powietrznych \textbf{DDDS}, ze szczególnym
...
Charakterystyczna sygnatura akustyczna silnika spalinowego Shahed-136 ---
tonalny pas harmonicznych powyżej fundamentu około 100\,Hz --- przedstawia
\rysref{rys:spectrum}. To właśnie ta wąska, periodyczna struktura widmowa
(w odróżnieniu od szerokopasmowego świstu wirników multicopterów) jest
głównym deskryptorem wykorzystywanym przez klasyfikator DDDS.

\begin{figure}[H]
\centering
\figureifexists{images/chart_shahed_spectrum.png}{0.95\linewidth}
\caption{Symulowana sygnatura akustyczna Shahed-136 odpowiadająca silnikowi
spalinowemu pracującemu przy ok.~6000\,RPM. Fundamentalna częstotliwość
$f_0 \approx 100$\,Hz oraz $\sim$30 silnych harmonicznych do około 3\,kHz
tworzą charakterystyczny tonalny ,,grzebień'' łatwo rozróżnialny od
szerokopasmowego tła miejskiego.}
\label{rys:spectrum}
\end{figure}

Przedmiotowy projekt \textbf{DDDS} jest \emph{programowo-laboratoryjnym}
...
Pozycjonowanie projektu DDDS na tle wymienionych rozwiązań przedstawia
\tabref{tab:positioning}.

\begin{table}[H]
\centering
\caption{Pozycjonowanie projektu DDDS na tle innych rozwiązań detekcji UAV.}
\label{tab:positioning}
\begin{tabularx}{\linewidth}{lXXXX}
\toprule
\textbf{Cecha} & \textbf{DDDS} & \textbf{Sky Fortress} & \textbf{Komercyjny radar} & \textbf{Squarehead} \\
\midrule
...
\begin{table}[H]
\centering
\caption{Pięciowarstwowy model architektury DDDS.}
\label{tab:warstwy}
...
System realizuje dwa równoległe potoki przetwarzania współdzielące te same
moduły ekstrakcji cech i klasyfikacji (\rysref{rys:dataflow}).
\emph{Potok treningowy} przebiega offline od plików MP3 dostarczonych przez
użytkownika do natrenowanego modelu i raportu ewaluacyjnego. \emph{Potok
inferencji} pracuje online, przyjmując strumień z mikrofonu USB i emitując
zdarzenia detekcji do dashboardu.
...
\begin{tikzpicture}[
  ...
  blk/.style    ={rectangle, rounded corners=2pt, draw=accent,
                  fill=accent!6, line width=0.5pt,
                  minimum width=1.8cm, minimum height=0.75cm,
                  align=center, inner sep=2pt},
  src/.style    ={blk, draw=cool,   fill=cool!12},
  model/.style  ={blk, draw=accent2,fill=accent2!12},
  outpos/.style ={blk, draw=success,fill=success!12},
  outdet/.style ={blk, draw=danger, fill=danger!12},
  ar/.style     ={-{Stealth[length=1.7mm]}, line width=0.45pt, draw=black!55},
  lab/.style    ={font=\scriptsize\bfseries\itshape},
]
% TRAINING row
\node[src]                       (t0) {audio/*.mp3};
\node[blk,    right=0.32cm of t0] (t1) {Ingest\\ffmpeg};
\node[blk,    right=0.32cm of t1] (t2) {Chunk\\2 s · 50\,\%};
\node[blk,    right=0.32cm of t2] (t3) {MFCC\\120-D};
\node[model,  right=0.32cm of t3] (t4) {RF\\train};
\node[outpos, right=0.32cm of t4] (t5) {reports/\\eval.md};

% INFERENCE row
\node[src,    below=1.1cm of t0] (l0) {USB mic};
\node[blk,    right=0.32cm of l0] (l1) {Resample\\$f_s\!\to\!22050$};
\node[blk,    right=0.32cm of l1] (l2) {Window\\2 s};
\node[blk,    right=0.32cm of l2] (l3) {MFCC\\120-D};
\node[model,  right=0.32cm of l3] (l4) {RF\\predict};
\node[outdet, right=0.32cm of l4] (l5) {Streamlit\\banner};

% Row labels on left
\node[lab, color=success, anchor=east] at ($(t0.west)+(-0.20,0)$) {TRAIN};
\node[lab, color=danger,  anchor=east] at ($(l0.west)+(-0.20,0)$) {LIVE};
...
% Shared-module emphasis (dashed bracket around MFCC + RF)
\draw[dashed, draw=accent!60, line width=0.4pt, rounded corners=2pt]
    ($(t3.north west)+(-0.08,0.18)$) rectangle
    ($(l4.south east)+(0.08,-0.18)$);
\node[lab, color=accent, anchor=south]
    at ($(t3.north)!0.5!(t4.north)+(0,0.22)$) {współdzielony stos cech + model};
\end{tikzpicture}
\caption{Dwa równoległe potoki przetwarzania DDDS współdzielące moduły ekstrakcji
cech (MFCC 120-D) oraz klasyfikator (Random Forest 300 drzew). Potok
\emph{TRAIN} działa wsadowo (offline) i kończy się raportem ewaluacyjnym;
potok \emph{LIVE} działa w czasie rzeczywistym i emituje zdarzenia detekcji
do dashboardu Streamlit.}
\label{rys:dataflow}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Zestawienie komponentów sprzętowych prototypu DDDS.}
\label{tab:hardware}
...
\begin{table}[H]
\centering
\caption{Stos technologiczny systemu DDDS.}
\label{tab:software}
\begin{tabularx}{\linewidth}{llX}
\toprule
...
Model wątkowy pętli real-time'owej przedstawia \rysref{rys:threading}.
...
\begin{tikzpicture}[
  >={Stealth[length=2mm]},
  every node/.style={font=\scriptsize},
  prod/.style={rectangle, rounded corners=3pt, draw=cool, fill=cool!12,
               minimum width=3.0cm, minimum height=2.1cm, align=center,
               inner sep=4pt},
  q/.style   ={rectangle, rounded corners=3pt, draw=accent2, fill=accent2!12,
               minimum width=2.4cm, minimum height=2.1cm, align=center,
               inner sep=4pt},
  cons/.style={rectangle, rounded corners=3pt, draw=accent, fill=accent!10,
               minimum width=3.8cm, minimum height=2.1cm, align=center,
               inner sep=4pt},
  ev/.style  ={rectangle, rounded corners=3pt, draw=danger, fill=danger!12,
               minimum width=3.4cm, minimum height=0.9cm, align=center,
               inner sep=2pt, font=\scriptsize\bfseries},
  ar/.style={-{Stealth[length=1.8mm]}, line width=0.55pt, draw=black!55},
]
\node[prod] (p) {%
  \textbf{Producent}\\[2pt]
  \scriptsize callback PortAudio\\
  \scriptsize (\texttt{sounddevice})\\[2pt]
  \scriptsize natywne $f_s = 48000$ Hz\\
  \scriptsize blocksize = $hop/4$};
\node[q, right=0.55 of p] (q) {%
  \textbf{queue.Queue}\\[2pt]
  \scriptsize bufor blokowy\\
  \scriptsize thread-safe\\[2pt]
  \scriptsize \texttt{put} / \texttt{get}};
\node[cons, right=0.55 of q] (c) {%
  \textbf{Konsument}\\[2pt]
  \scriptsize akumulacja w ring buffer\\
  \scriptsize resample $f_s\!\to\!22050$\\
  \scriptsize MFCC 120-D\\
  \scriptsize \texttt{model.predict\_proba}\\
  \scriptsize \texttt{yield DetectionEvent}};
\node[ev, below=0.55 of c] (e) {\texttt{DetectionEvent}\ \ $\longrightarrow$\ \ dashboard Streamlit};

\draw[ar] (p) -- (q);
\draw[ar] (q) -- (c);
\draw[ar] (c) -- (e);
\end{tikzpicture}
\caption{Model producent--konsument zaimplementowany w
\texttt{live/mic.py}. Wątek-producent (callback PortAudio) wpycha do
kolejki bufory wejściowe na natywnej częstotliwości próbkowania urządzenia
(typowo 48\,000\,Hz dla mikrofonów USB na Windows). Wątek-konsument drenuje
kolejkę, utrzymuje bufor pierścieniowy, resampluje każde okno do
częstotliwości treningowej modelu (22\,050\,Hz), oblicza 120-wymiarowy
wektor MFCC i emituje obiekt \texttt{DetectionEvent} co
\texttt{live\_hop\_seconds} (0{,}5\,s w konfiguracji domyślnej). Zdarzenie
trafia do dashboardu Streamlit, gdzie aktualizuje banner detekcji.}
\label{rys:threading}
\end{figure}
...
Schemat blokowy całego pipeline'u ekstrakcji cech prezentuje
\rysref{rys:mfcc}.
...
\begin{tikzpicture}[
  >={Stealth[length=2mm]},
  every node/.style={font=\scriptsize},
  s/.style={rectangle, rounded corners=2pt, draw=cool,   fill=cool!12,
            minimum width=1.55cm, minimum height=0.85cm, align=center, inner sep=2pt},
  t/.style={rectangle, rounded corners=2pt, draw=accent, fill=accent!8,
            minimum width=1.55cm, minimum height=0.85cm, align=center, inner sep=2pt},
  o/.style={rectangle, rounded corners=2pt, draw=accent2,fill=accent2!18,
            minimum width=1.85cm, minimum height=0.85cm, align=center, inner sep=2pt,
            font=\scriptsize\bfseries},
  ar/.style={-{Stealth[length=1.7mm]}, line width=0.45pt, draw=black!55},
]
\node[s]                       (in)   {Audio 2 s\\22050 Hz};
\node[t,  right=0.25 of in]    (pe)   {Pre-emph\\$0{,}97$};
\node[t,  right=0.25 of pe]    (stft) {STFT\\$N\!=\!2048$};
\node[t,  right=0.25 of stft]  (mel)  {Mel\\128 bin};
\node[t,  right=0.25 of mel]   (log)  {log power};
\node[t,  right=0.25 of log]   (dct)  {DCT 40\\$40\!\times\!87$};
\node[t,  right=0.25 of dct]   (agg)  {agg.\\$\mu, \sigma, \overline{\Delta\mu}$};
\node[o,  right=0.25 of agg]   (vec)  {wektor\\120-D};

\foreach \a/\b in {in/pe, pe/stft, stft/mel, mel/log, log/dct, dct/agg, agg/vec} {
  \draw[ar] (\a) -- (\b);
}
\end{tikzpicture}
\caption{Pipeline ekstrakcji 120-wymiarowego wektora cech MFCC. Z
2-sekundowego okna audio o częstotliwości próbkowania 22\,050\,Hz powstaje
macierz $40 \times 87$ współczynników cepstralnych. Agregacja po osi
czasowej daje trzy bloki po 40 wartości: średnią $\mu$, odchylenie
standardowe $\sigma$ oraz średnią pierwszej pochodnej temporalnej
$\overline{\Delta\mu}$. Konkatenacja produkuje końcowy 120-D wektor cech
podawany na wejście klasyfikatora.}
\label{rys:mfcc}
\end{figure}
...
\textbf{Surowe sygnały z 4 mikrofonów.} Aby algorytm miał z czego liczyć
opóźnienia, musi zarejestrować ten sam impuls akustyczny na wszystkich
czterech kanałach. \rysref{rys:4mic} pokazuje syntetyczny przykład dla
kwadratowego szyku o bazie 30\,cm i źródła o azymucie 30°. Ten sam ostry
impuls trafia do mikrofonu M3 najwcześniej (jest najbliżej źródła), do M1
najpóźniej (jest najdalej). Różnice są \emph{mikrosekundowe}
(setki~$\mu$s, kilkadziesiąt próbek przy 48\,kHz) --- to właśnie ta
sub-milisekundowa rozdzielczość czasowa wymaga sample-synchronicznego
przechwytywania na poziomie sprzętowym (sekcja końcowa rozdziału).

\begin{figure}[H]
\centering
\figureifexists{images/chart_4mic_waveforms.png}{0.95\linewidth}
\caption{Ten sam impuls akustyczny zarejestrowany jednocześnie przez 4
mikrofony szyku kwadratowego o bazie 30\,cm. Niebieskie linie przerywane
oznaczają chwilę przyjścia szczytu impulsu; różnice względem mikrofonu
referencyjnego M1 wynoszą od $-400$ do $-1200$\,$\mu$s, co odpowiada
21--57~próbkom przy $f_s = 48$\,kHz. Z tych mikroróżnic algorytm GCC-PHAT
wyznacza wektor TDOA $(\tau_{12}, \tau_{13}, \tau_{14})$, który następnie
linearyzuje się do układu na kierunek przyjścia fali $\hat{\mathbf{u}}$
(opisane poniżej).}
\label{rys:4mic}
\end{figure}

\textbf{Jak z tych przesunięć dostajemy kierunek?} W trzech krokach:
(1)~każdą parę mikrofonów $(i,j)$ przepuszczamy przez GCC-PHAT i
otrzymujemy estymatę $\hat\tau_{ij}$ --- pojedynczą liczbę w ms;
(2)~dla 4 mikrofonów mamy 6 par, czyli 6 równań na 2 niewiadome (kąt
azymutu i elewacji) --- układ \emph{nadokreślony}, rozwiązywany metodą
najmniejszych kwadratów; (3)~wynikowy wektor jednostkowy
$\hat{\mathbf{u}}$ jednoznacznie wyznacza kierunek przyjścia fali w
...
te warunki są w warunkach polowych nietrywialne. Wynik takiej korelacji ma
postać wąskiego, ostrego piku w okolicy faktycznego opóźnienia ---
\rysref{rys:gcc} prezentuje typowy kształt $R_{ij}(\tau)$ dla pary
mikrofonów z poprzedniej ilustracji.

\begin{figure}[H]
\centering
\figureifexists{images/chart_gcc_phat.png}{0.92\linewidth}
\caption{Przykładowa funkcja $R_{ij}(\tau)$ uzyskana metodą GCC-PHAT dla
jednej z 6 par mikrofonów. Pik główny w $\hat\tau = +0{,}45$\,ms odpowiada
faktycznemu opóźnieniu sygnału między mikrofonem $i$ a $j$. Mniejsze
,,duchy'' po bokach to artefakty pogłosu i szumu --- wybielenie fazą (PHAT)
skutecznie je tłumi w stosunku do klasycznej korelacji.}
\label{rys:gcc}
\end{figure}
...
i TDOA wszystkich 6 par. Wygląd strony \emph{4-mic TDOA concept} z bieżącymi
ustawieniami przedstawia \rysref{rys:tdoa_ui}.

\begin{figure}[H]
\centering
\figureifexists{images/screen_tdoa_concept.png}{\linewidth}
\caption{Strona \emph{4-mic TDOA concept} dashboardu DDDS.
Cztery karty KPI na górze (prędkość dźwięku, rozdzielczość kątowa
$\Delta\varphi = 0{,}89^\circ$, maksymalne TDOA, długość fali dla wybranej
częstotliwości), interaktywna wizualizacja geometrii szyku kwadratowego z
bieżącym kierunkiem źródła oraz frontem fali padającej, tabela TDOA dla
wszystkich sześciu par mikrofonów (w milisekundach oraz w próbkach przy
$f_s = 22\,050$\,Hz i $f_s = 48\,000$\,Hz). Wszystkie wykresy aktualizują
się w czasie rzeczywistym wraz z ruchem siedmiu suwaków w panelu bocznym.}
\label{rys:tdoa_ui}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Dekompozycja latencji systemu DDDS (mikrofon Maono, RPi4 lub
laptop, sieć lokalna).}
\label{tab:latency}
\begin{tabular}{lr}
\toprule
...
Najważniejszą wizualnie stroną dashbordu jest strona \emph{Live microphone},
której wygląd w stanie \emph{CLEAR} przedstawia \rysref{rys:live_ui}.

\begin{figure}[H]
\centering
\figureifexists{images/screen_live_mic.png}{\linewidth}
\caption{Strona \emph{Live microphone} dashboardu DDDS w trybie pracy
(stan CLEAR, $P(\text{shahed}) = 0{,}44$, próg detekcji $T = 0{,}50$,
mikrofon \emph{Microphone Array 2 (Realtek HD Audio)}, $f_s = 16\,000$\,Hz,
4 kanały). Centralnie zielony banner alarmowy z animacją CSS
\texttt{pulse\_clear}; poniżej w trzech kolumnach: gauge Plotly z aktualną
wartością $P(\text{shahed})$ (44\,\%) i strzałką delty względem progu,
poziome paski prawdopodobieństw per-klasy oraz miernik VU pokazujący
poziom wejściowy w dBFS ($-59{,}5$\,dB). U dołu karta KPI \emph{inference rate}
z bieżącym taktowaniem inferencji (2\,fps przy oknie 2000\,ms). W stanie
detekcji banner zmienia się na czerwony z animacją \texttt{pulse\_detect}.}
\label{rys:live_ui}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Metryki wydajnościowe systemu DDDS.}
\label{tab:perf}
\begin{tabular}{lrrr}
\toprule
...
Macierz pomyłek dla powyższych wyników przedstawia \rysref{rys:cm}.

\begin{figure}[H]
\centering
\figureifexists{images/chart_confusion_matrix.png}{0.55\linewidth}
\caption{Macierz pomyłek klasyfikatora Random Forest na zbiorze testowym
($n = 147$ chunków, stratyfikowany podział 80/20). Wartości na diagonali
to poprawne klasyfikacje, poza diagonalą --- pomyłki. Z 76 prawdziwych
próbek klasy \emph{noise} 71 zostało poprawnie odrzuconych (5 fałszywych
alarmów), a z 71 próbek \emph{shahed} 64 zostało rozpoznanych (7 nieuda\-nych
detekcji). Recall dla klasy pozytywnej wynosi $\approx 90$\,\%, a precision
$\approx 93$\,\%.}
\label{rys:cm}
\end{figure}
...
Projekt DDDS stanowi praktyczny wkład w obszar \emph{accessible defense
...
\bibitem{repo}
O.~Kropyva,
\textit{drone-detector --- DDDS acoustic UAV detection system},
GitHub, 2026.
